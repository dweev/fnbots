...
export const command = {
  ...
  execute: async ({ fn, toId, user, args, serial, sReply }) => {
    ...
    try {
      ...
      const winningNumberStr = Math.floor(Math.random() * 10000)
        .toString()
        .padStart(4, '0');
      let menang = false;
      let multiplier = 0.0;
      if (betType === '4d' && betNumbers === winningNumberStr) {
        menang = true;
        multiplier = 4000;
      } else if (betType === '3d' && betNumbers === winningNumberStr.slice(1)) {
        menang = true;
        multiplier = 400;
      } else if (betType === '2d' && betNumbers === winningNumberStr.slice(2)) {
        menang = true;
        multiplier = 70;
