...
userSchema.virtual('mutedUsersCount').get(function () { return this.mutedUsers.length; });
userSchema.virtual('isVIPActive').get(function () { return !!(this.isVIP && this.vipExpired && this.vipExpired > new Date()); });
userSchema.virtual('isPremiumActive').get(function () { return !!(this.isPremium && this.premiumExpired && this.premiumExpired > new Date()); });

const xpThresholds = [0, 1250, 3800, 5400, 7600, 9300, 12000, 18000, 24000, 30000];
userSchema.virtual('maxXp').get(function () { return xpThresholds[this.level] || 50000; });

const levelNames = [null, 'Beginner', 'Intermediate', 'Public', 'Pro', 'Expert', 'Master', 'Grandmaster', 'Epic', 'Legend'];
userSchema.virtual('levelName').get(function () { return levelNames[this.level] || 'Mythic'; });
