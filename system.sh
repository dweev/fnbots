  ...
  if [ "$DETAILED" = true ]; then
    if [ "$HAS_PM2" = "true" ]; then
      ...
      if [ -n "$PM2_INFO" ]; then
        ...
        echo ""
        echo "Log Files:"
        echo "  Error : /root/.pm2/logs/$PM2_NAME-error.log"
        echo "  Output: /root/.pm2/logs/$PM2_NAME-out.log"
        echo ""
        echo "═══════════════════════════════"
        echo "Commands:"
...
function logs_bot() {
  local LIMIT="${1:-10}"
  local LOG_FILE="/root/fnbots/logs/app_activity.log"
