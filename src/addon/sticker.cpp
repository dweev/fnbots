...
static SwsContext* MakeSws(int sw, int sh, AVPixelFormat sfmt, int dw, int dh){
  return sws_getContext(sw, sh, sfmt, dw, dh, AV_PIX_FMT_RGBA, 
...
static std::vector<uint8_t> RGBAResize512(const uint8_t* rgba, int w, int h, bool crop){
  ...
  if (crop){
    ...
    std::vector<uint8_t> cropped(side * side * 4);
    for (int y = 0; y < side; ++y) {
      std::memcpy(
        cropped.data() + y * side * 4,
        rgba + ((srcY + y) * w + srcX) * 4,
        side * 4
      );
    }

    SwsGuard sw;
    sw.p = MakeSws(side, side, AV_PIX_FMT_RGBA, TW, TH);
    ensure_ptr(sw.p, "sws_getContext crop failed");

    const uint8_t* srcSlice[4] = { cropped.data(), nullptr, nullptr, nullptr };
    int srcStride[4] = { side * 4, 0, 0, 0 };

    std::vector<uint8_t> out(TW * TH * 4);
    uint8_t* dstSlice[4] = { out.data(), nullptr, nullptr, nullptr };
    int dstStride[4] = { TW * 4, 0, 0, 0 };

    int ret = sws_scale(sw.p, srcSlice, srcStride, 0, side, dstSlice, dstStride);
    ensure(ret > 0, "sws_scale crop failed");
    
    return out;
  } else {
    double rw = (double)TW / w;
    double rh = (double)TH / h;
    double scale = std::min(rw, rh);
    int dstW = std::max(1, (int)(w * scale));
    int dstH = std::max(1, (int)(h * scale));

    SwsGuard sw;
    sw.p = MakeSws(w, h, AV_PIX_FMT_RGBA, dstW, dstH);
    ensure_ptr(sw.p, "sws_getContext pad failed");

    const uint8_t* srcSlice[4] = { rgba, nullptr, nullptr, nullptr };
    int srcStride[4] = { w * 4, 0, 0, 0 };

    std::vector<uint8_t> scaled(dstW * dstH * 4);
    uint8_t* dstSlice[4] = { scaled.data(), nullptr, nullptr, nullptr };
    int dstStride[4] = { dstW * 4, 0, 0, 0 };

    int ret = sws_scale(sw.p, srcSlice, srcStride, 0, h, dstSlice, dstStride);
    ensure(ret > 0, "sws_scale pad failed");
    ...
    for (int y = 0; y < dstH; ++y) {
      std::memcpy(
        canvas.data() + ((offY + y) * TW + offX) * 4,
        scaled.data() + y * dstW * 4,
        dstW * 4
      );
    }
    return canvas;
...
static std::vector<uint8_t> EncodeWebPStaticRGBA512(const uint8_t* rgba512, int quality){
  WebPConfig cfg; 
  ensure(WebPConfigPreset(&cfg, WEBP_PRESET_PICTURE, (float)quality), "WebPConfigPreset failed");
  ensure(WebPValidateConfig(&cfg), "Invalid WebP config");
  
  WebPPicture pic; 
  ensure(WebPPictureInit(&pic), "WebPPictureInit failed");
  pic.use_argb = 1; 
  pic.width = 512; 
  pic.height = 512;
...
static std::vector<RGBAFrame> DecodeAll(AVFormatContext* fmt, int si, AVCodecContext* dec,
  ...
  while (av_read_frame(fmt, &pkt) >= 0){
    ...
    while (true){
      ...
      int ret = sws_scale(sws.p, frame.p->data, frame.p->linesize, 
                          0, dstH, dstSlice, dstStride);
      ensure(ret > 0, "sws_scale decode failed");
  ...
  while (avcodec_receive_frame(dec, frame.p)==0){
    ...
    sws_scale(sws.p, frame.p->data, frame.p->linesize, 0, dstH, dstSlice, dstStride);
