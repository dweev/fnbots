...
public:
  ...
  : Napi::AsyncWorker(env), deferred_(def), url_(std::move(url)) {
    ...
    timeoutMs_      = getInt(opts, "timeout", 0);
    ...
    if (opts.Has("headers") && opts.Get("headers").IsObject()) {
      ...
      haveAccept_      = h.Has("Accept") || h.Has("accept");
    }
  ...
  void Execute() override {
    ...
    curl_easy_setopt(easy.get(), CURLOPT_SSL_ENABLE_NPN, 1L);
#endif
...
private:
  ...
  bool haveUserUA_ = false, haveAcceptEnc_ = false, haveConn_ = false, haveExpect_ = false, haveContentType_ = false, haveAccept_ = false;
