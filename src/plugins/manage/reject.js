...
export const command = {
  ...
  execute: async ({ fn, m, sReply, isBotGroupAdmins, dbSettings, toId, args, store }) => {
    ...
    if (pendingList.length === 0) return await sReply('Saat ini tidak ada permintaan bergabung yang tertunda.');
    ...
    if (jidsToReject.length === 0) return await sReply(`Tidak ada JID yang valid ditemukan dari input: "${fullSelector}"\n\nPending list: ${pendingList.length} orang`);
